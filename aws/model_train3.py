...
nb_train_samples = 127000
nb_validation_samples = 49000
...
save_best_model = ModelCheckpoint(filepath='3_model_.{epoch:02d}_{val_loss:.2f}.hdf5', verbose=1,
        monitor='val_loss')
...
model.add(Conv2D(filters=512, kernel_size=3, strides=1, activation='elu', kernel_initializer='glorot_normal'))
model.add(MaxPooling2D(pool_size=2, padding='same'))
...
fp = open('3_train_data.dat','w')
fp.write(res)
fp.close()

# Save model
model.save_weights('3_full_model_weights.h5')
