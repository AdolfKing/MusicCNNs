...
model.add(Dense(512, input_shape=(59,)))
...
def generate_arrays_from_file(generator):
    ...
    for x,y in generator:
        ...
        new_x = np.zeros( (len(x), 59) )
        # 256x256 => 1x59
        for i,xx in enumerate(x):
            image_array = xx
            uniform_array=lbp.lbp_uniform(image_array)
            xx = lbp.get_uniform_hist(uniform_array)
            new_x[i] = xx
        #print(new_x.shape)
        yield (new_x,y)
...
fp = open('lbp_mlp_train_data.dat','w')
fp.write(res)
fp.close()

# Save model
model.save_weights('lbp_mlp_full_model_weights.h5')
model.save('lbp_mlp_model.h5')
